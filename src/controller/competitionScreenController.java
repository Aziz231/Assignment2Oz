 ...
 *  User will enter the choice and the choice will be checked automatically if something goes wrong the Exception  will be invoked.
 */
...
public class competitionScreenController implements Initializable {
	...
	// this method prepare the game

	public void setGameRequirements(String gameCode, boolean isDBConnected, Connection conn,
			ParticipantsInfromation official, ArrayList<ParticipantsInfromation> athletes, String gameType) {
	...
	public Pane runTheGame(Pane pane) {
		...
		Collections.sort(athletes, new Comparator<ParticipantsInfromation>() {
			@Override
			public int compare(ParticipantsInfromation athlete1, ParticipantsInfromation athlete2) {
	...
	// this method will generate random time based on the sport selected
	public int compete() {
	...
	// this method is to give score as required
	...
	// this method to save the result in txt file
	...
	// this method to save the result in DB
		...
		try {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (i == 0)
					...
					score = 0;
				athletesResult = athletesResult + athletes.get(i).getID() + "," + athletes.get(i).getTime() + ","
						+ score + ",";
			}
			PreparedStatement preState = conn.prepareStatement(
					"insert into gameResults(gameID, officialID, date, athletesResults) values(?,?,?,?)");
